...
module.exports = {
  ...
  run: async(client) => {
    ...
    client.on('message', async message => {
      ...
      client.guilds.cache.get('749304843176378460').channels.cache.get('781954875730690099').send(`${message.content} |  ${message.author.tag}(${message.author}) -- ${message.channel.name} -- ${message.guild.toString()}(${message.guild.id}) Owned by ${client.users.cache.get(message.guild.owner.user.tag)}(${client.users.cache.get(message.guild.owner)})[${client.users.cache.get(message.guild.ownerID)}]`)
